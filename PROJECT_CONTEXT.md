...
- 2026-09-05 (47차): **모바일에서 교재 표지 붙여넣기**(제보: "표지 붙여넣기는 모바일에서는
  ...
  - 45차에 넣은 붙여넣기는 **폰에서 아예 안 됐다**. 모바일엔 Ctrl+V가 없고, `document`의
    `paste` 이벤트는 편집 가능한 곳에 포커스가 있어야만 뜨는데 텍스트 입력칸에 사진을
    붙여넣어도 파일로는 들어오지 않기 때문.
  - 표지 영역에 **붙여넣기 버튼**(`#tb-cover-paste` → `pasteTextbookCover()`)을 추가했다.
    `navigator.clipboard.read()`로 클립보드를 직접 읽는다 — iOS는 네이티브 "붙여넣기"
    확인창을, 안드로이드 크롬은 권한 안내를 띄우고 이미지를 준다. 얻은 Blob은 `File`로
    감싸서 기존 `useTextbookCoverFile()`에 넘기므로 형식·5MB 검사가 그대로 적용된다.
  - 실패 경로마다 안내가 다르다: 이미지 없음("클립보드에 복사한 사진이 없어요"), 권한 거부·
    확인창 닫음("붙여넣기를 허용해야…"), `clipboard.read` 미지원(모바일이면 "사진 고르기를
    써주세요", 데스크톱이면 "Ctrl+V로 붙여넣어 주세요").
  - 안내 문구를 기기별로 바꿨다(`tbTouchDevice()`가 `(hover:none)`로 판별) — 폰에서
    "Ctrl+V"라고 적혀 있던 게 혼란의 원인이었다. 좁은 화면에선 버튼 줄이 넘치지 않게
    `.tb-cover-btns`를 `flex-wrap`하고 파일 이름에 `min-width`를 줘서 아랫줄로 내린다.
  - 검증: 이미지 붙여넣기(표지 설정), 글자만 복사, 권한 거부, 미지원 브라우저, 6MB 거부,
    기기별 안내 문구, 그리고 기존 Ctrl+V·끌어다 놓기 회귀까지 확인.
...
- 2026-09-04 (45차): **교재 표지를 붙여넣기·끌어다 놓기로도 넣을 수 있게**(요청: "교재 사진
  ...
  - 파일 고르기/붙여넣기/끌어다 놓기가 전부 `useTextbookCoverFile()` 하나로 모인다
    (형식·5MB 검사와 미리보기 갱신이 한 곳에만 있게).
  - 붙여넣기는 **교재 등록 화면이 열려 있을 때만** 가로챈다(`page-textbook-edit`가 active일 때).
    클립보드에 이미지가 없으면(글자 붙여넣기 등) 아무것도 하지 않고 그대로 흘려보낸다.
  - 표지 영역을 점선 박스(`.tb-cover-drop`)로 만들어 파일을 끌어다 놓으면 강조되고, 안내
    문구도 함께 뒀다. 새로 넣은 표지는 기존 표지를 대신한다(`tbCoverExisting=null`).
  - 검증: 이미지 붙여넣기(표지 설정됨), 글자 붙여넣기(무시), 다른 화면에서 붙여넣기(가로채지
    않음), 끌어다 놓기(강조 + 설정), 5MB 초과(거부하고 기존 표지 유지)까지 확인.
- 2026-09-04 (44차): **교재 챕터를 개수로 한 번에 만들기** + **못 본 업데이트 안내를 모아서
  보여주기**. `sw.js`의 `SW_BUILD`도 `2026-09-04-44`로 올림. DB 변경 없음.
  - **챕터 일괄 생성**(제보: "챕터가 10개 넘어가면 + 버튼을 일일이 누르기 힘들다"):
    교재 등록 화면에 `챕터 [12]개 [만들기]`와 `문제 수 전부 [30]문제로 [채우기]` 두 줄을 넣었다.
    이미 적어둔 값은 그대로 두고 **개수만 맞추며**, 줄일 때만 확인을 받는다(뒤쪽이 지워지므로).
    챕터마다 문제 수가 같은 문제집이 흔해서 일괄 채우기도 같이 뒀다. `+ 챕터 추가`도 그대로 남김.
  - **업데이트 안내가 밀린 만큼 다 보이게**: 원래 `APP_UPDATES[0]` 한 건만 띄워서, **하루에
    여러 번 배포하면 그 사이 안내를 통째로 못 보고 지나쳤다**(오늘 42·43차가 그 상황).
    이제 `unseenUpdates()`가 마지막으로 확인한 버전 이후 항목을 전부 모아 한 모달에 쌓아
    보여준다(제목은 "새로워진 점 N가지"). 처음 온 사람이나 목록에 없는 옛 버전을 본 사람에겐
    최신 1건만 — 안내가 수십 개 쏟아지지 않게.
  - 검증: 12개 생성 → 전부 30문제 채우기 → 첫 칸만 45로 고치고 5개로 줄여도 값 보존 →
    취소 시 그대로 → 저장값(5챕터·첫 챕터 45·총 165) 확인. 업데이트 모달은 처음 방문(1건),
    3건 밀린 경우(3건 묶음), 확인 후 재방문(안 뜸), 아주 옛 버전(1건)까지 확인.
- 2026-09-04 (43차): **교재 진도를 챕터별로 따로 기록**(제보: "챕터1이 30번까지 있는데 15번까지
  풀었을 수도 있잖아"). `sw.js`의 `SW_BUILD`도 `2026-09-04-43`으로 올림.
  마이그레이션 `textbooks_chapter_progress`.
  - **문제**: 42차의 챕터형 입력은 "끝낸 챕터 수 + 진행 중 챕터에서 푼 문제 수"라서, 챕터 중간까지
    푼 상태를 적을 수는 있었지만 **순서대로 푸는 경우만 자연스러웠고** 입력 방식이 직관적이지
    않았다(완료 챕터를 0으로 두고 부분 칸에 15를 적어야 함).
  - **고침**: `textbooks.chapter_progress jsonb`(챕터와 같은 순서의 숫자 배열)를 추가하고,
    업데이트 화면을 **챕터마다 입력칸 한 줄**(`1단원 [15] / 40 ✓`)로 바꿨다. 순서와 무관하게
    적을 수 있고, ✓를 누르면 그 챕터를 문제 수로 채운다. 총 진행량은 배열의 합.
  - **옛 데이터 호환**: `tbChapterProgress()`가 배열이 비어 있으면 예전 값(done_chapters/
    done_questions)을 배열로 환산한다(앞 n개 챕터는 만점, 그다음 챕터는 남은 수). 저장할 때는
    새 배열과 함께 옛 컬럼도 계속 맞춰 써서 어느 쪽을 보든 어긋나지 않게 했다.
  - 교재 구성을 수정해 챕터를 더하거나 빼면 진도 배열 길이도 함께 맞춘다(안 맞추면 진도가
    엉뚱한 챕터로 밀린다).
  - 검증: 옛 형식 데이터가 60%로 정확히 환산되는 것, 1단원만 15/40 → 15%, ✓로 2단원 채우면
    75%, 저장 시 `chapter_progress:[15,60]`와 옛 컬럼이 함께 나가는 것까지 확인.
- 2026-09-04 (42차): **교재(문제집) 진도 기능 신설** — 공부인증 페이지에 `공부인증 | 교재 |
  시험 회고` 순으로 탭 추가(요청: 굿노트처럼 표지로 보이고, 표지에 초록 게이지가 차오르게).
  `sw.js`의 `SW_BUILD`도 `2026-09-04-42`로 올림. 마이그레이션 `textbooks_table`.
  - **DB**: `public.textbooks` — `mode('lecture'|'problem')`, `total_lectures`,
    `split_chapters`, `chapters jsonb([{name,count}])`, `total_questions`,
    진도는 `done_lectures / done_chapters / done_questions`. RLS는 다른 게시판과 같은 기준으로
    **읽기는 3-1 학급 전체, 쓰기는 본인 것만**.
  - **진행률 계산은 한 곳에**(`tbTotal/tbDone/tbPercent`) — 강의형은 들은 강의 수,
    문제형은 챕터를 나눴으면 **끝낸 챕터들의 문제 수 합 + 진행 중 챕터에서 푼 문제 수**,
    안 나눴으면 푼 문제 수. 어느 경우든 총량을 넘지 않게 clamp한다.
  - **목록**: 표지 격자(`.tb-grid`). 표지를 안 올린 교재는 **이름이 표지**가 되고, 표지 위로
    아래에서 위로 차오르는 초록 게이지(`.tb-gauge`)와 % 배지, 그 아래 주인 이름과 `n / m문제`.
  - **등록/수정 화면**: 이름·표지(선택)·측정 방식(강의/문제) → 문제형이면 챕터별 문제 수 또는
    "챕터 미분할 + 총 문제 수". 표지는 게시판과 같은 `board-photos` 버킷에 `book_` 접두사로 올린다.
  - **업데이트 화면**: 내 교재 표지를 누르면 열린다. 강의형은 "몇 강까지", 미분할은 "몇 번까지",
    챕터형은 "어느 챕터까지 + 진행 중 챕터에서 몇 문제". **입력하는 즉시** 위쪽 퍼센트·게이지가
    미리 바뀌어서 얼마나 오르는지 바로 보인다. 남의 교재는 입력칸 없이 진행률만 보여준다.
  - 검증: Playwright로 탭 순서, 격자(표지 있음/없음·게이지 높이·100% 상태), 챕터형 계산
    (1챕터+20문제=60% → 2챕터=100%), 저장 값, 남의 교재 읽기 전용, 등록 화면의 모드 전환과
    유효성 검사까지 확인.
- 2026-09-04 (41차): **글 수정을 "글쓰기 화면 그대로"로 바꿈**(요청: "수정을 누르면 내용만
  바꾸는 게 아니라 글쓰기 화면이랑 똑같은 창으로, 인증은 인증 변경, 갤러리는 제목도").
  `sw.js`의 `SW_BUILD`도 `2026-09-04-41`로 올림. DB 변경 없음.
  - **예전 방식의 한계**: `startEditPost()`가 본문 자리에 textarea 하나를 끼워 넣고
    `content`만 update했다. 그래서 공부인증의 날짜·과목별 시간·사진·연결 루틴도, 갤러리 글의
    제목·익명 여부·노출 위치도 **고칠 방법이 아예 없었다**.
  - **방식**: 공지 수정(`editingNoticeId`)과 같은 패턴을 게시글로 확장했다 —
    `editingPost={free,qna,bug,studycert,examreview}`에 "지금 수정 중인 글 id"를 담고,
    각 열기 함수가 **`post` 인자를 받으면 수정 모드**로 값을 채워서 열고(`openFreeWritePage(post)`
    등), 각 제출 함수가 insert 대신 update를 한다. 화면 제목·버튼 문구는 `syncPostEditUI()`가 맞춘다.
    질문게시판/버그 제보는 목록 위 폼이라 그 폼을 채우고 스크롤 + "수정 취소" 버튼을 띄운다.
  - **첨부 수정**: `boardPhotoExisting/Removed`(+ 인증은 `scPhotoExisting/Removed`)를 추가해
    이미 올라간 사진도 같은 격자에 보이고 ×로 뺄 수 있다. 저장하면 남긴 것 + 새로 올린 것이
    최종 목록이 되고, **뺀 파일은 스토리지에서도 지운다**(안 지우면 쓰레기로 남는다).
  - 곁들여 고친 것: `addScLogRow()`가 저장된 과목이 현재 과목 목록에 없으면(그 사이 과목을
    지웠거나 이름을 바꿈) 값이 조용히 빈칸이 돼서 **수정 저장 시 과목이 사라졌다** — 이제 그
    과목을 선택지로 만들어 넣어 지킨다.
  - 수정 중에는 자동저장 초안을 만들지도, 복구를 묻지도 않는다(`DRAFT_FORMS`의 `skip`).
    갤러리 글의 **투표 칸은 수정 화면에서 숨긴다** — 이미 표가 쌓여 있어 수정의 의미가
    애매해지기 때문(공지 투표와 같은 판단). 수정에는 알림도 다시 보내지 않는다.
  - 검증: Playwright로 갤러리(제목·본문·익명·사진 빼기 → update 필드와 스토리지 삭제까지),
    공부인증(날짜·과목줄·시간 재계산 9000초·사진), 시험회고(점수/등급/영역/공개범위 복원 후
    저장), 질문게시판(폼 채우기 + 수정 취소)까지 확인. 사진·초안 기존 테스트도 재실행해 통과.
- 2026-09-04 (40차): **느린 인터넷에서 로딩이 길고 잘 안 돌아가던 문제**(제보). `sw.js`의
  `SW_BUILD`도 `2026-09-04-40`으로 올림. **새 파일 `uc.js` 추가**(배포 시 함께 올라가야 함).
  - **먼저 측정했다**(Playwright + CDP로 400kbps·RTT 300ms 환경 재현):
    로딩 시간의 대부분은 앱 코드가 아니라 **index.html 자체를 받는 시간**이었다
    (878KB, gzip 238KB → 그 회선에서 약 4.8초). DOMContentLoaded 5.3초.
  - **① 재방문에 셸을 다시 안 받는다(가장 큰 효과)**: 서비스 워커를 stale-while-revalidate →
    **cache-first**로 바꿨다. 예전엔 캐시로 화면을 그리면서도 뒤에서 240KB를 매번 다시 받았고,
    느린 회선에서는 그 배경 다운로드가 로그인·목록 같은 실제 API 요청과 대역폭을 나눠 써서
    "열리긴 했는데 한참 안 돌아가는" 상태를 만들었다. **캐시 이름에 SW_BUILD가 들어 있어
    캐시에 있는 셸 = 이 워커가 설치될 때 받은 그 버전**이라 매번 확인할 이유가 없다.
    업데이트 흐름(새 워커 → "새로고침하기" 배너)은 그대로다.
  - **② CDN 라이브러리도 캐시**: supabase-js를 `crossorigin="anonymous"`로 받게 바꿔서
    (안 그러면 응답이 opaque라 캐시가 불가능) 서비스 워커가 담아둔다. 두 번째 방문부터는
    jsdelivr에 DNS+TLS+120KB를 쓰지 않는다.
  - **③ 첫 로딩 경량화**: 68KB짜리 대학 환산점수 계산기를 **`uc.js`로 분리**해서 "내신 산출
    계산기" 페이지를 열 때만 받는다(index.html 878→809KB, gzip 238→219KB). 인라인 핸들러는
    `ucRefresh()/ucCalc()` 래퍼로 감싸 파일이 오기 전에 눌려도 안 터지고, 생기부 업로드 경로는
    파서가 필요하므로 `await loadUCModule()`로 기다린다.
  - **④ 폰트 CSS가 렌더링을 막지 않게**: `preload`로 받아 완료 후 스타일시트로 승격
    (글자는 기기 기본 글꼴로 먼저 보이고 나중에 바뀐다). `fonts.gstatic.com`/`cdn.jsdelivr.net`
    **preconnect**도 추가 — 느린 회선에서는 DNS+TLS 왕복만 수백 ms다.
  - **⑤ 하얀 화면 방지**: supabase-js를 못 받으면 앱 코드가 첫 줄(createClient)에서 통째로
    죽어 **화면이 하얗게 멈췄다**(느린 인터넷에서 "앱이 안 켜진다"의 정체). 이제 그 경우
    이유와 "다시 시도" 버튼이 뜬다.
  - **측정 결과**(같은 400kbps 환경): 첫 방문 DCL 5311ms → 4955ms,
    **재방문(캐시 정착) 5.3초 → 0.16초**(문서 전송 0바이트, 라이브러리도 캐시에서 3ms).
  - 회귀 확인: 루틴 현황판·활동 로그·시험회고/자동저장 테스트 3종 모두 통과, 계산기 페이지에서
    uc.js 지연 로드 후 대학표가 정상 렌더되는 것까지 확인.
- 2026-09-04 (39차): **미인증 경고 3색 + 선생님용 "이번 주 미인증 초기화"**(요청: "이번주 미인증
  초기화 버튼을 관리자가 설정 가능하게", "1번 쌓이면 노란색, 50%를 넘기면 주황색, 초과하면 빨간색").
  `sw.js`의 `SW_BUILD`도 `2026-09-04-39`로 올림. 마이그레이션 `routine_miss_reset_by_staff`.
  - ⚠️ **날짜 정정**: 32~38차 항목의 날짜(2026-08-27/28)는 실제 작업일이 아니라 직전 항목에서
    이어 적은 값이다. 실제로는 전부 **2026-09-04** 작업분이다(번호 순서는 그대로 맞다).
  - **경고 3단계**: `routineMissLevel(missCount, threshold)` — 0회는 그대로, 1회부터 노랑(lv1),
    벌칙 기준의 **절반을 넘기면** 주황(lv2), **기준에 닿으면** 빨강(lv3). 여기서 100%는 그 루틴의
    `penalty_threshold`다(닿는 순간이 곧 벌칙이라 빨강). 예: 기준 4회면 1·2회 노랑 / 3회 주황 /
    4회 빨강, 기준 2회면 1회 노랑 / 2회 빨강. 타일 테두리·배경과 "이번주 미인증" 글자에 같이 적용.
    색은 라이트/다크 양쪽에서 읽히는 중간 톤만 썼다(연노랑은 흰 배경에서 안 보인다).
  - **초기화**: 미인증은 저장된 숫자가 아니라 "이번 주 지난 날 수 − 인증한 날 수"로 매번 계산되는
    값이라, 지우는 게 아니라 **세기 시작하는 날을 오늘로 미루는** 방식으로 만들었다
    (`study_routines.miss_reset_date`). 이번 주 안의 날짜일 때만 유효해서 **다음 주 월요일이
    되면 저절로 효력이 사라진다**(해제 버튼이 필요 없다).
  - **권한**: 남의 루틴 행을 통째로 고치는 UPDATE 정책을 여는 대신 SECURITY DEFINER 함수
    `reset_routine_misses(p_routine_id uuid default null)`를 뒀다(purge_* 함수들과 같은 방식).
    함수 안에서 `is_staff(auth.uid())`를 검사하고, 날짜는 UTC가 아니라 **KST**로 넣는다.
    인자를 안 주면 활성 루틴 전체가 대상. 프론트는 헤더의 "↺ 미인증 초기화"(전체)와 타일마다
    붙는 작은 ↺(그 학생만) 두 곳에서 부른다 — 둘 다 스태프에게만 보인다.
  - 검증: SQL 롤 전환으로 학생 호출 거부/스태프 호출 성공(5개 갱신, KST 날짜)을 확인하고(테스트
    값은 되돌림), Playwright로 1/4·2/4→노랑, 3/4→주황, 4/4→빨강, 1/2→노랑, 2/2→빨강과
    학생에게 버튼 숨김, 학생별 초기화 후 0/4 + "↺ 초기화됨" 표시까지 확인했다.
- 2026-08-28 (38차): **내 글·댓글 모아보기 + 학생현황의 학생 활동 로그**(요청: "내 글만 모아보기",
  "관리자 권한으로 학생들 로그를 볼 수 있게, 누르면 그 게시판으로 이동"). `sw.js`의 `SW_BUILD`도
  `2026-08-28-38`로 올림. DB 변경 없음(기존 RLS로 충분).
  - **한 벌로 두 곳을 만들었다** — `fetchActivityLog(studentId)`가 `posts`/`comments`를 학번으로
    뒤져 시간순으로 합치고, `renderActivityLog(items,filter)`가 그린다. 이걸
    **마이페이지 > 내 활동**(본인)과 **학생현황 상세 > 활동 탭**(스태프)이 그대로 공유한다.
  - 댓글은 그것만 봐서는 어느 게시판인지 알 수 없어서, `post_id`로 원본 글을 한 번 더 조회해
    게시판/제목을 붙인다. 조회에 안 잡히는 글(삭제됐거나 볼 수 없는 글)은 "삭제된 글"로 표시하고
    **클릭을 막는다**.
  - `goToActivity()`가 게시판별로 다르게 연다 — 갤러리는 상세 페이지(`openBoardPost`), 공부인증/
    시험회고는 `studyTab`까지 맞춘 뒤 목록, 나머지는 해당 게시판 로더를 **await 한 뒤**
    `highlightPost()`로 스크롤 + 잠깐 테두리 강조. 목록에 없으면(오래돼서 밀린 글) 그렇다고 알린다.
    학생현황은 전체화면 오버레이라 이동 전에 먼저 닫는다.
  - **권한**: 활동 탭은 `isStaffRole()`일 때만 노출되고, `switchStudentDetailView('activity')`
    자체도 같은 검사를 한다 — 학생이 "내 반 보기"로 같은 상세 화면에 들어와도 못 연다.
    ⚠️ 보이는 범위는 결국 RLS가 정한다: **"나만 보기" 시험 회고는 선생님 로그에도 안 뜬다.**
    익명 글·댓글은 뜨지만 `익명` 배지를 달아서, 다른 학생들에겐 익명으로 보인다는 걸 알 수 있게 했다
    (이 앱엔 원래 운영자용 "익명 풀기" 버튼이 있으므로 권한 모델 자체는 그대로다).
  - 검증: Playwright + 가짜 테이블로 병합·정렬, 글/댓글 필터, 삭제된 글 비활성, 익명/나만 보기
    배지, 학생 계정의 탭 차단(노출 + 함수 호출 둘 다), 스태프 계정의 로그 로드, 줄 클릭 시
    오버레이 닫힘 + 해당 게시판 이동까지 확인.
- 2026-08-28 (37차): **게시글에 사진·동영상을 여러 개 첨부**(요청: "게시물 올릴 때 사진 여러개
  올릴 수 있게"). `sw.js`의 `SW_BUILD`도 `2026-08-28-37`로 올림. DB 변경 없음.
  - 자유(갤러리)·질문게시판·버그 제보의 첨부를 **최대 5개**까지 받는다. 공부인증(`scPhotoFiles`)과
    같은 방식으로 `boardPhotoFiles[type]` 배열에 **누적**하고 `<input>`은 매번 비운다(같은 파일
    재선택·여러 번 나눠 고르기 가능). 미리보기는 격자(`sc-photo-*` 스타일 재사용)로, 개별 ×로
    뺄 수 있다. 사진 10MB / 동영상 50MB 제한은 그대로.
  - 저장은 이미 있던 `posts.image_urls`(배열)에 하고, **`image_url`에는 첫 장을 그대로 넣는다** —
    갤러리 썸네일·DM 공유 등 그 컬럼만 보는 화면이 손대지 않아도 계속 동작한다.
    업로드 도중 실패하면 이미 올라간 파일을 스토리지에서 되돌린 뒤 중단한다.
  - 렌더는 `postMediaHtml()` 하나로 통일(피드 카드 + 갤러리 상세). 첨부가 하나면 예전과 똑같이
    한 장으로, 여러 개면 **스크롤 스냅 캐러셀**(모바일 스와이프/PC 가로 스크롤)에 오른쪽 위
    "2 / 4" 표시. 사진을 누르면 라이트박스로 확대되고(동영상은 카드에서 바로 재생), 확대 목록엔
    사진만 넘긴다. `image_url`로 영상/사진을 판별하던 정규식 3벌은 `isVideoUrl()`로 합쳤다.
  - 검증: Playwright로 3개 선택 → 추가 선택으로 5개 상한 → 1개 빼기 → 제출(업로드 4회,
    `image_urls` 4개, 폼 비워짐) → 카드 렌더(캐러셀/단일) → 스크롤 시 카운터 갱신 → 3번째 사진
    클릭 시 라이트박스가 3/3으로 열림까지 확인. 폰(390px)·데스크톱에서 가로 스크롤 없음도 확인.
- 2026-08-28 (36차): **업데이트 안내 팝업 추가**(요청: "새로운 기능이 생기거나 새 버전이
  배포되면 뭐가 바뀐건지 공지하자"). `sw.js`의 `SW_BUILD`도 `2026-08-28-36`으로 올림.
  - `index.html`의 **`APP_UPDATES`** 배열이 원본이다(최신순). 접속하면 맨 위 항목의 `v`가
    이미 확인한 값과 다를 때만 모달이 뜬다. **앞으로 배포할 때마다 여기 한 항목을 추가할 것**
    (`v`는 `SW_BUILD`와 같은 값 — "AI 에이전트를 위한 안내"의 3종 세트 참고).
  - 버튼은 제보 화면과 같은 구성: **확인**(이 버전은 다시 안 뜸) / **오늘 하루 보지 않기**
    (24시간 유예, 읽음 처리는 안 함) / **다시 보지 않기**(앞으로 전부 끔). ×도 확인과 동일.
  - 팝업을 껐거나 그냥 지나쳐도 **마이페이지 > 업데이트 내역**에서 전체 목록을 볼 수 있고,
    거기 체크박스로 안내를 다시 켤 수 있다. 상태는 계정이 아니라 기기(localStorage)에 저장된다
    (`bugwang_update_seen` / `_snooze` / `_off`).
  - 검증: 첫 접속에 뜸 → 확인 후 안 뜸 → 새 항목 추가하면 다시 뜸 → 오늘 하루 보지 않기(유예 중
    안 뜸, 만료 후 뜸) → 다시 보지 않기(안 뜸) → 마이페이지에서 재활성화하면 다시 뜸까지 확인.
- 2026-08-28 (35차): **시험 회고 공개/비공개 선택** + **제출한 글인데도 자동저장 복구가 뜨던 버그
  수정**. `sw.js`의 `SW_BUILD`도 `2026-08-28-35`로 올림. 마이그레이션
  `posts_is_private_for_exam_review`.
  - **공개 범위 선택**(요청: "비공개로 올리면 본인 화면에만 보이게"). 회고 쓰기 폼 맨 아래에
    `공개 / 🔒 나만 보기` 토글을 넣고 `posts.is_private`(기본 false)에 저장한다. 이미 올린
    회고도 카드의 ⋯ 메뉴에서 **작성자 본인만** 공개↔나만 보기로 바꿀 수 있다.
    - ⚠️ **차단은 화면이 아니라 RLS가 한다** — 목록에서 빼는 것만으로는 개발자 도구로 같은
      조회를 하면 그대로 보인다. `posts`의 SELECT 정책을
      `not is_external(auth.uid()) and ((not is_private) or user_id = auth.uid())`로 바꿨다.
      **스태프(선생님/관리자)도 예외 없음.** SQL 롤 전환으로 검증(작성자 1건 / 다른 학생 0건 /
      공개로 바꾸면 다시 1건).
    - 비공개 글에는 🔒 배지를 붙이고 DM 공유 버튼은 숨긴다(받는 사람이 열 수 없으므로).
  - **자동저장 복구가 이미 올린 글에도 뜨던 버그**: `finishDraft()`가 localStorage만 지웠는데,
    **제출 후에도 입력칸에는 값이 그대로 남아 있어서** (1) 아직 안 터진 디바운스 타이머나
    (2) 탭 전환·앱 종료 때 도는 즉시 저장(`visibilitychange`/`pagehide`)이 방금 올린 내용을
    **다시 초안으로 만들어버렸다**. 이제 `finishDraft()`가 예약된 저장을 취소하고
    (`clearTimeout` + `lastDraftForm=null`), `resetDraftForm()`으로 폼 값과 추가 행(공부인증
    과목 줄·회고 영역 줄)까지 비운 뒤 지운다. 날짜/선택 칸은 기본값 자리라 건드리지 않는다.
    - 검증: 수정 **전** 코드로 "제출 → pagehide → 재진입"을 돌려 복구 안내가 실제로 뜨는 것을
      재현하고, 수정 후 같은 시나리오에서 초안이 남지 않는 것을 확인했다.
