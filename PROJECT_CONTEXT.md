...
- 2026-09-08 (56차): **대역폭 초과 후속 — 캐시 헤더 1년 + DM 서명URL 재사용**.
  55차(사진 축소)는 *앞으로 올릴* 사진만 줄인다. 이미 올라간 621MB가 반복 송신되는 원인을
  두 개 더 찾아 고쳤다. `SW_BUILD`도 `2026-09-08-56`으로 올림.
  - **① 캐시 헤더가 `max-age=3600`이었다**(Supabase 기본값). 파일명에 타임스탬프+난수가 박혀
    있어 내용이 절대 안 바뀌는데도 **1시간마다 다시 내려받고** 있었다. "캐시 송신 26.66GB"의
    큰 축. 신규 업로드는 `UPLOAD_OPTS={cacheControl:'31536000'}`(1년)로 올린다.
    아바타는 파일명이 고정이라 덮어쓰기지만, URL 뒤에 `?t=`가 붙어 있어 1년 캐시라도 새로
    올리면 즉시 반영된다.
  - **이미 올라간 273개 파일도 고쳤다** — 재업로드 없이
    `update storage.objects set metadata=jsonb_set(metadata,'{cacheControl}','"max-age=31536000"')`.
    서비스가 제한된 상태에서도 DB 쓰기는 살아 있어서 지금 적용 가능했다.
  - **② DM 재렌더 때마다 사진 전부를 다시 받고 있었다.** 상대에게 새 메시지가 오면
    `pollDmActiveThread` → `loadDmMessages` → `renderDmThreadMessages`로 대화창을 통째로
    다시 그리는데, 그때 `hydrateDmPhotoUrls`가 보이는 사진 전부에 **새** 서명URL을 발급했다.
    URL이 매번 달라 브라우저 캐시가 한 번도 안 맞았다 — 사진 20장짜리 방이면 메시지 하나 올
    때마다 26MB.
    - `dmSignedUrls`(path → {url, exp})에 발급분을 들고 있다가 재사용한다. 만료 1분 전부터만
      다시 받는다. `dmMessageRowHtml`도 아는 URL이면 처음부터 `src`를 박아 깜빡임을 없앴다.
    - 측정: 사진 5장 방에서 재렌더 3번 → 서명 호출이 매번 5건씩 15건이던 것이 **새 사진 1건씩**
      으로 줄었다. 만료 시엔 전체 재발급되는 것도 확인.
  - 남은 것: 55차와 동일 — 기존 사진 자체의 용량(621MB)은 그대로다. 아바타 12개를 다시 올리면
    가장 효과가 크다. simo-materials 평균 22MB도 위험.
- 2026-09-08 (55차): **Supabase 대역폭 초과로 서비스 중단 — 원인 규명 + 업로드 사진 자동 축소**
  (제보: 로그인 시 콘솔에 402 반복). `SW_BUILD`도 `2026-09-08-55`로 올림.
  - **증상**: `/auth/v1/token`만 402, REST·DB는 200 정상. 즉 로그인만 막힘.
  - **원인 확인 방법**: 에이전트 프록시가 `*.supabase.co` 직접 호출을 막아서, `pg_net`으로
    DB 안에서 auth 엔드포인트를 때려 402 **본문**을 읽었다:
    `"Service for this project is restricted due to the following violations:
    exceed_cached_egress_quota, exceed_egress_quota."`
    (조직 메일도 동일 — 송출 6.29GB/5.5GB, **캐시 송신 26.66GB/5.5GB**, 9/12 리셋)
  - **왜 26GB나 나갔나**: 폰 원본 사진을 그대로 올려서 그대로 내려받게 하고 있었다.
    스토리지 실측 — board-photos 199개 419MB(평균 **2.1MB**, 최대 14MB),
    dm-photos 57개 74MB, avatars 12개 18MB(평균 **1.5MB**), simo-materials 5개 110MB(평균 22MB).
    저장 621MB가 반복 조회되며 캐시 송신만 26.66GB. **아바타가 특히 나쁨** — 댓글·DM·인증
    타일마다 뜨는데 1.5MB짜리를 24px로 줄여 그린다.
  - **조치**: `shrinkImage(file, kind)` 신설 — 캔버스로 줄이고 JPEG로 다시 굽는다.
    한도는 `IMG_LIMITS` (avatar 512px/q.85, board·dm 1600px/q.82, cover 900px/q.85).
    적용: 아바타·게시판 사진·공부인증 사진·DM 사진·교재 표지. 동영상은 건드리지 않는다.
    - 안전장치: GIF·SVG는 그대로(구우면 움직임이 사라짐), 이미 작고 300KB 이하면 그대로,
      결과가 원본보다 크면 원본 사용, 디코딩 실패(HEIC 등)면 원본 사용 —
      **사진을 못 올리게 되는 것보다 큰 파일이 올라가는 게 낫다.**
    - 줄이면 확장자가 .jpg가 되므로 **확장자 계산을 축소 뒤로** 옮겼다. 게시판 루프는
      `const file`에 재할당하려다 터져서 `picked`/`file`로 분리했다.
  - 측정: 4032x3024 1787KB → 134KB(93%↓), 아바타 2000px 1214KB → 39KB(97%↓),
    표지 1067KB → 106KB(90%↓), DM 1419KB → 206KB(85%↓), PNG 스크린샷 1983KB → 248KB(88%↓),
    작은 사진·GIF·깨진 파일은 원본 그대로 통과 확인.
  - **남은 것**: 이미 올라간 621MB는 그대로다(재인코딩 안 함). 아바타 12개만 다시 올려도
    반복 송신이 크게 준다. Supabase 이미지 변환(`/render/image/...?width=`)을 쓰면 재업로드
    없이 해결되지만 유료 플랜 기능이라 확인 필요. simo-materials 평균 22MB도 위험 요소.
- 2026-09-06 (54차): **교재 가져오기 + 자주 쓰는 교재 미리 등록**(제보: "교재 등록이 번거롭다.
  다른 친구가 등록한 교재면 따올 수 있게, 많이 쓰는 교재는 미리 등록하고 싶다").
  `SW_BUILD`도 `2026-09-06-54`로 올림.
  - **DB 마이그레이션 `textbook_presets`**: 운영자가 미리 올려두는 교재 목록
    (title/cover_url/mode/total_lectures/split_chapters/chapters/total_questions/sort_order).
    RLS는 읽기 `not is_external()`, 쓰기·수정·삭제는 `is_staff()`.
  - 교재 탭 헤더에 **📚 가져오기**. 한 목록에 두 갈래를 같이 보여준다 —
    ⭐ 자주 쓰는 교재(프리셋)와 👥 친구들이 등록한 교재(제목 기준으로 묶고 "n명이 쓰는 중"
    표시). 내 교재는 목록에서 빼고, 내가 이미 같은 제목을 가졌으면 "이미 있음"을 붙인다.
    같은 제목이 여럿이면 **표지가 있는 행을 대표로** 삼는다(표지 없는 행이 대표가 되면
    그림이 안 따라온다).
  - 고르면 `useTextbookSetup()`이 등록 화면을 그 설정으로 채워서 연다. **바로 저장하지 않는
    이유**: 같은 문제집도 판형에 따라 문항 수가 달라서 확인할 여지를 남긴다.
    `tbSetupOf()`가 설정만 뽑으므로 **진도는 따라오지 않는다**(남의 진도를 내 것으로
    가져오면 안 되니까).
  - 목록 클릭 핸들러에는 **인덱스만** 넘긴다(`tbPickItems`). 처음엔
    `escAttr(JSON.stringify(...))`로 설정을 속성에 박았는데, 제목에 따옴표가 들어가면
    (`김동욱 '독서' 기출`) 깨진다.
  - 운영자는 교재 등록 화면의 **⭐ 자주 쓰는 교재에 넣기**(`addTextbookPreset()`)로 지금 입력한
    설정을 프리셋에 올리고, 가져오기 목록에서 × 로 뺄 수 있다(학생에겐 둘 다 안 보인다).
  - 검증: 프리셋/친구 교재 두 그룹 렌더, 검색, 챕터형 가져오기(챕터 2개 그대로 · 표지 승계 ·
    저장 payload에 진도 없음), 따옴표 제목 + 강의형 가져오기, 운영자만 × 와 ⭐ 버튼,
    실제 DB에서 학생 읽기 O / 학생 쓰기 차단 / 운영자 쓰기 O까지 확인.
