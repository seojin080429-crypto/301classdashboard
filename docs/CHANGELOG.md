# 부광고 3-1 대시보드 — 변경 기록 보관소

> `PROJECT_CONTEXT.md`의 "최근 변경사항"에서 넘어온 지난 기록입니다(최신순).
> 최근 5건은 `PROJECT_CONTEXT.md`에 그대로 있습니다.
>
> **이 파일은 통째로 읽지 마세요.** 95건 · 311KB라 전부 읽으면 컨텍스트를 크게 잡아먹습니다.
> 찾을 게 있으면 `grep -n "키워드" docs/CHANGELOG.md`로 줄 번호를 찾고 그 주변만 보세요.
